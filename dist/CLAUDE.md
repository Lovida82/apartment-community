...
- **사용자 탭**: 사용자 관리 (검색, 필터링, 상세보기)
  - 닉네임/카카오ID 검색
  - 관심사/동 필터링
  - 상세 모달: 기본정보, 팔로워/팔로잉, 관심사 전체 목록, 차단 목록
- **모임방 탭**: 모임방 관리
- **이벤트 탭**: 이벤트 관리
- **공지사항 탭**: 공지 작성/삭제
- **모임후기 탭**: 게시글 관리
...
### 2026-02-03
- **프로젝트 외장하드 이동 대응**
  - `.netlify/netlify.toml` publish 경로를 상대경로(`dist`)로 변경
  - Git safe.directory 설정 추가
- **관리자 페이지 사용자 관리 기능 대폭 개선**
  - 사용자 검색 기능 (닉네임, 카카오ID)
  - 필터링 기능 (관심사별, 동별)
  - 사용자 상세 정보 모달 추가
    - 기본정보 (동, 연령대, 성별, 가입일)
  ...
  - 사용자 카드에 관심사 미리보기 표시
- **보안 개선**
  - CLAUDE.md에서 Firebase API 키 노출 제거
  - Google Cloud Console API 키 HTTP 리퍼러 제한 설정 권장
- **관심사 한글 매핑 완성**
  - 관리자 페이지의 모든 탭(사용자, 모임방, 이벤트, 게시글)에서 관심사 한글 표시
  - index.html의 interestCategories와 동기화 (45개 관심사)
